...
class _TaskGridState extends State<TaskGrid> {
  ...
  void initState() {
    ...
    futureTasks = getTasks(
        widget.isPending, widget.userId); // Call the async function here
  }
                        ...
                        child: Padding(
                          ...
                          child: Column(
                              ...
                              mainAxisSize: MainAxisSize
                                  .min, // Ensures the card only wraps its contents
                              children: [
                                Text(
                                  ...
                                  maxLines:
                                      1, // Limits to 1 line, so it only shows the ellipsis after the first line
                                ),
                                Text(
                                  tasks[index].assignedTo,
                                  style: TextStyle(
                                      color: theme.darkgrey, fontSize: 15),
                                ),
                                Text(
                                  tasks[index].dueDate,
                                  style: TextStyle(
                                ...
                                widget.isPending
                                    ...
                                    : Row(
                                        ...
                                        children: [
                                          ...
                                          ElevatedButton(
                                            onPressed: () {
                                              deletePressed(tasks[index].taskId,
                                                  widget.userId, index);
                                            },
                                            style: ElevatedButton.styleFrom(
                                              backgroundColor: theme.darkblue,
  ...
  Future<bool> markCompleted(
      String taskId, String userId, int taskIndex) async {
    bool markedSuccess = false;
    try {
      print(taskId);
      print(userId);
      var reqBody = {"frm": userId, "id": taskId};
      print(reqBody);
      var response = await http.patch(Uri.parse(markComplete),
          headers: {"Content-Type": "application/json"},
  ...
  void reuseTaskPressed(
      String taskName, String assignedTo, String taskId, String dueDate) {
  ...
  void deletePressed(String taskId, String userId, int taskIndex) async {
    try {
      ...
      var response = await http.delete(Uri.parse(deleteTask),
          headers: {"Content-Type": "application/json"},
          body: jsonEncode(reqBody));
