...
describe("UserPresistence Class -- Getting a room id", () => {
    ...
    it("Should return an error that user doesn't have a room yet", async () => {
        await expect(user_persistence.get_room_id(no_room_user_id))
        .rejects.toThrow(`User ${no_room_user_id} doesn't have a room yet`);
    });
});
...
describe("UserPersistence Class -- Getting notifications", () => {
    ...
    it("Should return an error that user doesn't have a notification yet", async () => {
        await expect(user_persistence.get_notification(no_notif_user_id))
        .rejects.toThrow(`User ${no_notif_user_id} doesn't have a notification yet`);
    });
});
